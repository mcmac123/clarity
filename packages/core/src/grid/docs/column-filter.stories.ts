  ...
  class DemoColumnFilter extends LitElement {
    ...
    render() {
      ...
      <cds-grid aria-label="column filter datagrid demo" height="360">
        ...
        <cds-grid-column>
          ${column.label}
          ${i === 0 ? html`<cds-button-action popup="column-filter" @click=${(e: any) => (this.anchor = e.target)} shape="filter" .pressed=${!!this.search.length} aria-label="search ${column.label}"></cds-button-action>` : ''}
        </cds-grid-column>`)}
