      ...
      return html`
        <cds-grid aria-label="columns fixed dynamic datagrid demo" height="360">
          <cds-grid-column resizable .position=${this.pinFirst ? 'fixed' : ''} width="150">
            ${this.firstColumn.label} <cds-button-action @click=${() => (this.pinFirst = !this.pinFirst)} .pressed=${this.pinFirst} shape=${this.pinFirst ? 'pin' : 'unpin'} aria-label="pin ${this.firstColumn.label} column"></cds-button-action>
          </cds-grid-column>
          ...
          <cds-grid-column width="150" resizable .position=${this.pinLast ? 'fixed' : ''}>
            ${this.lastColumn.label} <cds-button-action @click=${() => (this.pinLast = !this.pinLast)} .pressed=${this.pinLast} shape=${this.pinLast ? 'pin' : 'unpin'} aria-label="pin ${this.lastColumn.label} column"></cds-button-action>
          </cds-grid-column>
