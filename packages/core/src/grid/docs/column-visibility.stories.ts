    ...
    render() {
      return html`
        <cds-grid aria-label="column visibility datagrid demo" height="360">
          ...
          <cds-grid-footer>
            <cds-button-action popup="column-visbility" @click=${(e: any) => (this.columnAnchor = e.target)} aria-label="filter column" shape="view-columns" .pressed=${!this.checked(this.all)}></cds-button-action>
          </cds-grid-footer>
        </cds-grid>
