      ...
      return html`
        <cds-grid aria-label="Active VM Management" height="360">
          ...
          <cds-grid-column resizable width="180">
            Host <cds-button-action popup="id-filter" @click=${(e: any) => (this.state = { ...this.state, idFilterAnchor: e.target })} aria-label="column filter options" shape="filter" .pressed=${!!this.state.search}></cds-button-action>
          </cds-grid-column>
          ${this.columnVisible(ColumnTypes.Status) ? html`
          <cds-grid-column resizable width="180">
            Status <cds-button-sort aria-label="sort status" .sort=${this.state.sortType} @sortChange=${(e: any) => this.setSortType(e.detail)}></cds-button-sort>
          </cds-grid-column>`: ''}
          ...
          <cds-grid-row .selected=${entry.selected}>
            ...
            <cds-grid-cell>
              <cds-button-expand popup="row-detail" action="detail" aria-label="${entry.id} details" .pressed=${this.currentDetail?.id === entry.id} @click=${(e: any) => this.showDetail(entry.id, e.target)}></cds-button-expand>
            </cds-grid-cell>
          ...
          <cds-grid-footer>
            <cds-button-action popup="column-visibility" @click=${(e: any) => (this.state = { ...this.state, columnsDropdownAnchor: e.target })} aria-label="filter columns" shape="view-columns" .pressed=${!!this.state.search}></cds-button-action>
            <cds-grid-pagination
