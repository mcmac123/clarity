    ...
    render() {
      return html`
        <cds-grid aria-label="row detail datagrid demo" height="360">
          ...
          <cds-grid-row>
            <cds-grid-cell>
              <cds-button-expand popup="row-detail" action="detail" aria-label="${row.id} details" .pressed=${this.selectedRow?.id === row.id} @click=${(e: any) => this.showDetail(e.target, row)}></cds-button-expand>
            </cds-grid-cell>
