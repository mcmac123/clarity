...
export class TouchController<T extends Touch> implements ReactiveController {
  ...
  async hostConnected() {
    ...
    this.host.addEventListener('mousedown', (e: any) => this.start(e), false);
    this.host.addEventListener('touchstart', (e: any) => this.start(e), false);
  }

  private start(e: any) {
    if (e.path.find((el: any) => el === this.host)) {
      this.startPosition = getPointerCoordinates(e);
      document.addEventListener('mousemove', this.resizeHandler, false);
      document.addEventListener('touchmove', this.resizeHandler, false);
      document.addEventListener('mouseup', this.endHandler, false);
